...
class Wav2Vec2Decoder:
    ...
    def greedy_decode(self, logits: torch.Tensor) -> str:
        ...
        """
        # <YOUR CODE GOES HERE>
        # 1. Берём самый вероятный токен на каждом таймстепе
        ...
        for idx in pred_ids:
            if idx != prev_id and idx != self.blank_token_id:
    ...
    def beam_search_decode(self, logits: torch.Tensor, return_beams: bool = False):
        ...
        for t in range(T):
            ...
            for prefix, (p_b, p_nb) in beams.items():
                for c in range(V):
                    ...
                    if c == self.blank_token_id:
                        ...
                        nb_p_b = _log_add(nb_p_b, p_b + p)
                        nb_p_b = _log_add(nb_p_b, p_nb + p)
                        new_beams[prefix] = (nb_p_b, nb_p_nb)
                        continue

                    last = prefix[-1] if prefix else None

                    if c == last:
                        # 1) Repeat without blank in between:
                        # stay in the same collapsed prefix
                        if prefix not in new_beams:
                            new_beams[prefix] = (float("-inf"), float("-inf"))

                        nb_p_b, nb_p_nb = new_beams[prefix]
                        nb_p_nb = _log_add(nb_p_nb, p_nb + p)
                        new_beams[prefix] = (nb_p_b, nb_p_nb)

                        # 2) Repeat after blank:
                        # now it becomes a genuine extension
                        new_prefix = prefix + (c,)
                        if new_prefix not in new_beams:
                            new_beams[new_prefix] = (float("-inf"), float("-inf"))

                        nb_p_b, nb_p_nb = new_beams[new_prefix]
                        nb_p_nb = _log_add(nb_p_nb, p_b + p)
                        new_beams[new_prefix] = (nb_p_b, nb_p_nb)
            ...
            for prefix, (p_b, p_nb) in new_beams.items():
                ...
                beam_items.append((prefix, (p_b, p_nb), total))

            beam_items.sort(key=lambda x: x[2], reverse=True)
            beam_items = beam_items[:self.beam_width]
            beams = {prefix: (p_b, p_nb) for prefix, (p_b, p_nb), _ in beam_items}

        final_beams = []
        for prefix, (p_b, p_nb) in beams.items():
            total = _log_add(p_b, p_nb)
            final_beams.append((list(prefix), total))
    ...
    def beam_search_with_lm(self, logits: torch.Tensor) -> str:
        ...
        for t in range(T):
            ...
            for prefix, (p_b, p_nb) in beams.items():
                for c in range(V):
                    ...
                    if c == last:
                        # Repeat without blank in between -> stay in same prefix
                        if prefix not in new_beams:
                            new_beams[prefix] = (float("-inf"), float("-inf"))
                        ...
                        # Repeat after blank -> real extension
                        new_prefix = prefix + (c,)
                        if new_prefix not in new_beams:
                            new_beams[new_prefix] = (float("-inf"), float("-inf"))
            ...
            # Score each beam with acoustic + shallow-fusion LM
            beam_items = []
            for prefix, (p_b, p_nb) in new_beams.items():
                acoustic_score = _log_add(p_b, p_nb)
                ...
                total_score = acoustic_score + self.alpha * lm_score + self.beta * num_words
                beam_items.append((prefix, (p_b, p_nb), total_score))
        ...
        for prefix, (p_b, p_nb) in beams.items():
            ...
            total_score = acoustic_score + self.alpha * lm_score + self.beta * num_words
            final_beams.append((list(prefix), total_score))
